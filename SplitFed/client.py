...
def main():
    ...
    common_config.local_epoch = client_config.common_config.local_epoch
    common_config.tag = 1
    # init config
...
async def local_training(comm, common_config, train_dataset, test_loader, labels):
    ...
    if labels:
        train_loader = datasets.create_dataloaders(train_dataset, batch_size=int(bsz), selected_idxs=train_data_idxes,
    ...
    if common_config.momentum < 0:
        ...
        optimizer = optim.SGD(local_model.parameters(), momentum=common_config.momentum, nesterov=True, lr=epoch_lr,
