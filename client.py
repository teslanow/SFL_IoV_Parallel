...
device = torch.device("cuda:%d" % (rank % 7 + 0) if args.use_cuda and torch.cuda.is_available() else "cpu")
if rank == 9:
    device = torch.device("cuda:4" if args.use_cuda and torch.cuda.is_available() else "cpu")
if rank == 10:
    device = torch.device("cuda:5" if args.use_cuda and torch.cuda.is_available() else "cpu")
# if rank <= 5:
#     device = torch.device("cuda:%d" % (rank % 4))
# else:
#     device = torch.device("cuda:%d" % (rank % 3 + 4))
